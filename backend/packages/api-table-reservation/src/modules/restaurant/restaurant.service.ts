...
import {CreateRestaurantDto, CreateRestaurantTableDto} from "./restaurant.dto";
...
export class RestaurantService {
  ...
  async getRestaurantById(id: string): Promise<{
    ...
    const tables = await this.masterConnection.getRepository(RestaurantTable).find({
      where: {
        restaurantId: id
      }
    })
  ...
  async createTable(restaurantId: string, table: CreateRestaurantTableDto): Promise<string> {
