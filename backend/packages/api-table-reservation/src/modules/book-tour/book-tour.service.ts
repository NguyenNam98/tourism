...
export class BookTourService {
  ...
  async getTourServiceByTourId(
      tourId: string,
  ): Promise<TourServices[]> {
  ...
  async bookATour(
    ...
    const tour = await this.masterConnection
        .getRepository(Tour)
        .findOne({
          where: {
            isValid: true,
    ...
    const bookingId = await this.masterConnection.getRepository(BookingTour).insert({
        userId,
        tourId,
  ...
  async getBookedTourByUserId(
      userId: string,
  ): Promise<{
    tours: Tour[],
    bookingTours: BookingTour[]
  }> {
    const bookingTours = await this.masterConnection.getRepository(BookingTour).find({
        where: {
            userId
        }
    })
