...
export class BookTourController {
  ...
  async createTour(@Body() tour: CreateTourDto): Promise<TBaseDto<string>> {
    return {
  ...
  @Get("/list")
  @UsePipes(ValidationPipe)
  async getListTourDetail(): Promise<TBaseDto<Tour[]>> {
    return {
  ...
  @Post("/book")
  @UsePipes(ValidationPipe)
  async bookTour(@Body() tour: CreateBookingDto): Promise<TBaseDto<string>> {
    const id = await this.tourService.bookATour(tour);
