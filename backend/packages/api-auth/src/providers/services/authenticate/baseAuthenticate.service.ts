...
import {accessTokenPayload, JWTTokenService} from "../../../services/JWTToken.service";
...
@Injectable()
export abstract class BaseAuthenticateService implements AuthenticateServiceInterface{
  ...
  private async createNewEmailUser(authRegisterUserDto: AuthRegisterUserDto): Promise<string> {
    ...
    const user = await this.masterConnection.getRepository(Auth).insert(dataBaseNewUser)
  ...
  private generateTokenPairs(tokenPayloadDefault: TDefaultPayload): {
    ...
    const accessToken = this.jwtTokenService.generateToken(atsPayload, ACCESS_TOKEN_JWT_LIFE_TIME)
  ...
  private async validateLogin(email: string, password: string): Promise<Auth> {
    ...
    try {
      ...
      if (!authData || password !== this.cryptoService.decrypt(authData.password)) {
  ...
  async login(authLoginUserDto: AuthLoginUserDto): Promise<TTokenPair> {
  ...
  private async createTokenHistory(uid: string, refreshToken: string): Promise<string> {
  ...
  async refreshToken(refreshToken: string): Promise<TRefreshTokenResponseDTO|null> {
    ...
    const accessToken =  this.jwtTokenService.generateToken(defaultPayload, ACCESS_TOKEN_JWT_LIFE_TIME)
